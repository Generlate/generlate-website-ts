...
const Header = (props: { useTheme: (arg: string) => void, theme:string , name: string, setName: (name: string) => void }) => {
  ...
  useEffect(() => {
    if (profilePictureFile) {
      ...
      fetch('http://generlate.azurewebsites.net/api/upload-user-images', {
        method: 'PUT',
        body: formData,
        credentials: 'include',
      })
  ...
  const switchTheme = () => {
      ...
      fetch('http://generlate.azurewebsites.net/api/update-user-color-theme', {
          method: 'PUT',
          body: formData,
          credentials: 'include',
        })
  ...
  const logout = async () => {
    await fetch("http://generlate.azurewebsites.net/api/logout", {
        method: "POST",
  ...
  if (props.name) {
    fetch('http://generlate.azurewebsites.net/api/user-data', {
      method: 'GET',
      credentials: 'include',
      headers: {
      ...
      .then(data => {
        ...
        const profilePictureUrl = 'http://generlate.azurewebsites.net' + userImage
