...
const Header = (props: { useTheme: (arg: string) => void, theme:string , name: string, setName: (name: string) => void }) => {
  ...
  useEffect(() => {
    if (profilePictureFile) {
      ...
      fetch('http://18.224.58.130:8000/api/upload-user-images', {
        method: 'PUT',
        body: formData,
        credentials: 'include',
      })
  ...
  const switchTheme = () => {
      ...
      fetch('http://18.224.58.130:8000/api/update-user-color-theme', {
          method: 'PUT',
          body: formData,
          credentials: 'include',
        })
  ...
  const logout = async () => {
    await fetch("http://18.224.58.130:8000/api/logout", {
        method: "POST",
  ...
  if (props.name) {
    fetch('http://18.224.58.130:8000/api/user-data', {
      ...
      .then(data => {
        ...
        const profilePictureUrl = 'http://18.224.58.130:8000' + userImage
